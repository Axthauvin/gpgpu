...
void write_png(const std::byte *buffer, int width, int height, int stride,
               const char *filename)
{
...
int main(int argc, char **argv)
{
    ...
    // Rendering
    spdlog::info("Runnging {} mode with (w={},h={},niter={}).", mode, width,
                 height, niter);
    if (mode == "CPU")
    {
        render_cpu(reinterpret_cast<char *>(buffer.get()), width, height,
                   stride, niter);
    }
    else if (mode == "GPU")
    {
